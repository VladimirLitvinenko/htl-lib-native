...
/**
 * @brief Метод ModeShort приводит к типу @a short значение типа @a htl::asio::DataType
 * @param mode приводимое значение типа @a htl::asio::DataType
 * @return результат типа @a short для @a type
 */
inline
short                             ModeShort( const htl::asio::DataType & mode )
 ...
 */
inline
std::size_t                      ModeSizeT( const htl::asio::DataType & mode )
...
using asio_signal_action_t      = boost::function<boost::system::error_code( int, boost::system::error_code &) >;
...
using asio_work_t               = boost::asio::io_context::work;
...
using asio_steady_ptr_t         = std::shared_ptr< boost::asio::steady_timer >;
...
using duplex_ports_t            = std::vector< htl::asio::port_t >;
...
//------------------------------------------------------------------------------
using asio_errcallback_t        = std::function < void ( const boost::system::error_code & ) >;
using asio_errcallbacks_t       = std::map< boost::system::errc::errc_t, asio_errcallback_t >;

using buffer_transformation_t   = std::function< vectors_data_t ( const vectors_data_t & in) >;
//------------------------------------------------------------------------------
inline
bool                              IsNoErr( const boost::system::error_code & err )
