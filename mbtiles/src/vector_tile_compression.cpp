...
void init_z_stream( const char * data, std::size_t size, z_stream & inflate_s )
{
...
void init_z_stream( z_stream & inflate_s )
{
  inflate_s.zalloc = Z_NULL;
  inflate_s.zfree = Z_NULL;
  inflate_s.opaque = Z_NULL;
...
{
    z_stream inflate_s;
    init_z_stream( data, size, inflate_s );
    size_t length = 0;
    do
    {
...
void
zlib_compress(const char * data, std::size_t size, std::string & output, bool gzip, int level, int strategy)
{
    z_stream deflate_s;
    init_z_stream( deflate_s );
    int windowsBits = 15;
    if (gzip)
    {
