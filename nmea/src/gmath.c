        ...
        )
{
    ...
    { /* Iterate */
        /* Calculation */
        double tmp1 = cos_U2 * sin_lambda;
        double tmp2 = cos_U1 * sin_U2 - sin_U1 * cos_U2 * cos_lambda;
        double sin_sigma = sqrt(tmp1 * tmp1 + tmp2 * tmp2);
        double cos_sigma = sin_U1 * sin_U2 + cos_U1 * cos_U2 * cos_lambda;
//        double tan_sigma = sin_sigma / cos_sigma;
        ...
        sqr_cos_2_sigmam = cos_2_sigmam * cos_2_sigmam;
        double C = f / 16 * sqr_cos_alpha * (4 + f * (4 - 3 * sqr_cos_alpha));
...
//    double tan_sigma1 = tan_U1 / cos_alpha1;
    sigma1 = atan2(tan_U1, cos_alpha1);
    sin_alpha = cos_U1 * sin_alpha1;
    sqr_cos_alpha = 1 - sin_alpha * sin_alpha;
//    double cos_alpha = sqrt(sqr_cos_alpha);
    sqr_u = sqr_cos_alpha * (sqr_a - sqr_b) / sqr_b;
    A = 1 + sqr_u / 16384 * (4096 + sqr_u * (-768 + sqr_u * (320 - 175 * sqr_u)));
    B = sqr_u / 1024 * (256 + sqr_u * (-128 + sqr_u * (74 - 47 * sqr_u)));
